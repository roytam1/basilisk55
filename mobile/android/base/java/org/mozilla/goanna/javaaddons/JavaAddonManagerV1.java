...
package org.mozilla.goanna.javaaddons;
...
public class JavaAddonManagerV1 implements BundleEventListener {
    private static final String LOGTAG = "GoannaJavaAddonMgrV1";
    ...
    private final org.mozilla.goanna.EventDispatcher mDispatcher;
    ...
    private JavaAddonManagerV1() {
        mDispatcher = org.mozilla.goanna.EventDispatcher.getInstance();
    }
    ...
    protected synchronized EventDispatcherImpl registerNewInstance(String classname, String filename)
        ...
        final File dexFile = GoannaJarReader.extractStream(mApplicationContext, filename, mApplicationContext.getCacheDir(), "." + extension);
        try {
            if (dexFile == null) {
    ...
    public synchronized void handleMessage(final String event, final GoannaBundle message,
    ...
    private class EventDispatcherImpl implements JavaAddonInterfaceV1.EventDispatcher {
        ...
        protected class ListenerWrapper implements BundleEventListener {
            ...
            @Override // BundleEventListener
            public void handleMessage(final String prefixedEvent, final GoannaBundle message,
                                      final EventCallback callback) {
        ...
        public synchronized void registerEventListener(
            ...
            mDispatcher.registerGoannaThreadListener(listenerWrapper, prefixedEvents);
        ...
        public synchronized void unregisterEventListener(
            ...
            mDispatcher.unregisterGoannaThreadListener(pair.first, pair.second);
        }
        ...
        @Override
        public void sendRequestToGoanna(final String event, final JSONObject message,
                                       final JavaAddonInterfaceV1.RequestCallback callback) {
            final String prefixedEvent = guid + ":" + event;
            GoannaAppShell.sendRequestToGoanna(new GoannaRequest(prefixedEvent, message) {
                @Override
                public void onResponse(NativeJSObject nativeJSObject) {
                    ...
                    try {
                        ...
                        callback.onResponse(GoannaAppShell.getApplicationContext(), json);
