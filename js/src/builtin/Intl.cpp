...
js::intl_patternForSkeleton(JSContext* cx, unsigned argc, Value* vp)
{
    ...
    JSString* str =
        CallICU(cx, [gen, &skeletonChars](UChar* chars, uint32_t size, UErrorCode* status) {
            return udatpg_getBestPattern(gen, skeletonChars.begin().get(), skeletonChars.length(),
                                         chars, size, status);
        });
    if (!str)
        return false;
...
intl_FormatToPartsDateTime(JSContext* cx, UDateFormat* df, double x, MutableHandleValue result)
{
    ...
    MOZ_ASSERT(overallResult->length() >= 0);
