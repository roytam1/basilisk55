...
# GoannaMigration {{{1
class GoannaMigration(MercurialScript, BalrogMixin, VirtualenvMixin,
        ...
        for k in ('from', 'to'):
            ...
            if url:
                self.goanna_repos.append({
                    ...
                    # goanna repo. see mozharness/base/vcs/mercurial.py for implementation
...
# Actions {{{1
    ...
    def pull(self):
        """ Pull tools first, then clone the goanna repos
            """
        repos = [{
