             ...
             implements BundleEventListener
{
    ...
    private SharedPreferences getSharedPreferences(final GoannaBundle message) {
     ...
     * and an Object value.
     */
    private void handleSet(final GoannaBundle message) {
        SharedPreferences.Editor editor = getSharedPreferences(message).edit();

        final GoannaBundle[] bundlePrefs = message.getBundleArray("preferences");

        for (int i = 0; i < bundlePrefs.length; i++) {
            final GoannaBundle pref = bundlePrefs[i];
     ...
     * "string"].
     */
    private GoannaBundle[] handleGet(final GoannaBundle message) {
        final SharedPreferences prefs = getSharedPreferences(message);
        final GoannaBundle[] bundlePrefs = message.getBundleArray("preferences");
        final GoannaBundle[] bundleValues = new GoannaBundle[bundlePrefs.length];

        for (int i = 0; i < bundlePrefs.length; i++) {
            final GoannaBundle pref = bundlePrefs[i];
            final String name = pref.getString("name");
            final String type = pref.getString("type");
            final GoannaBundle bundleValue = new GoannaBundle(3);
    ...
    private static class ChangeListener
        ...
        private static void putSharedPreference(final GoannaBundle msg,
        ...
        public void onSharedPreferenceChanged(final SharedPreferences sharedPreferences,
            ...
            final GoannaBundle msg = new GoannaBundle(5);
     ...
     * disable listening.
     */
    private void handleObserve(final GoannaBundle message) {
        final SharedPreferences prefs = getSharedPreferences(message);
