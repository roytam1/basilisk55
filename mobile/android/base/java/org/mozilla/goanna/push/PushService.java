...
package org.mozilla.goanna.push;
...
import org.mozilla.goanna.fxa.FxAccountDeviceRegistrator;
import org.mozilla.goanna.fxa.FxAccountPushHandler;
import org.mozilla.goanna.fxa.authenticator.AndroidFxAccount;
import org.mozilla.goanna.fxa.login.State;
import org.mozilla.goanna.gcm.GcmTokenClient;
 ...
 * <p/>
 * This singleton services Goanna messages from dom/push/PushServiceAndroidGCM.jsm and Google Cloud
 * Messaging requests.
 * <p/>
 * It is expected that Goanna is started (if not already running) soon after receiving GCM messages
 * otherwise there is a greater risk that pending messages that have not been handle by Goanna will
...
public class PushService implements BundleEventListener {
    private static final String LOG_TAG = "GoannaPushService";
    ...
    private static final String[] GOANNA_EVENTS = new String[] {
    ...
    public static synchronized void onCreate(Context context) {
        ...
        sInstance.registerGoannaEventListener();
        sInstance.onStartup();
    }
    ...
    public PushService(Context context) {
        ...
        pushManager = new PushManager(new PushState(context, "GoannaPushState.json"), new GcmTokenClient(context), new PushManager.PushClientFactory() {
            @Override
    ...
    public void onMessageReceived(final @NonNull Context context, final @NonNull Bundle bundle) {
        ...
        if (canSendPushMessagesToGoanna()) {
            if (!GoannaThread.canUseProfile(profileName, new File(profilePath))) {
                Log.e(LOG_TAG, "Mismatched profile for chid: " + chid + "; ignoring dom/push message.");
                return;
            }
        } else {
            final Intent intent = GoannaService.getIntentToCreateServices(context, "android-push-service");
            GoannaService.setIntentProfile(intent, profileName, profilePath);
        ...
        try {
            ...
            // Goanna handler will verify this.
            ...
            Log.e(LOG_TAG, "Got exception delivering dom/push message to Goanna!", e);
            return;
        }
        ...
        if (isWebPush) {
            sendMessageToGoannaService(data);
        } else {
            sendMessageToDecodeToGoannaService(data);
        }
    }

    protected static void sendMessageToGoannaService(final @NonNull JSONObject message) {
        Log.i(LOG_TAG, "Delivering dom/push message to Goanna!");
        GoannaAppShell.notifyPushObservers("PushServiceAndroidGCM:ReceivedPushMessage",
                                          message.toString());
    }

    protected static void sendMessageToDecodeToGoannaService(final @NonNull JSONObject message) {
        Log.i(LOG_TAG, "Delivering dom/push message to decode to Goanna!");
        GoannaAppShell.notifyPushObservers("FxAccountsPush:ReceivedPushMessageToDecode",
                                          message.toString());
    }

    protected void registerGoannaEventListener() {
        Log.d(LOG_TAG, "Registered Goanna event listener.");
        EventDispatcher.getInstance().registerBackgroundThreadListener(this, GOANNA_EVENTS);
    }

    protected void unregisterGoannaEventListener() {
        Log.d(LOG_TAG, "Unregistered Goanna event listener.");
        EventDispatcher.getInstance().unregisterBackgroundThreadListener(this, GOANNA_EVENTS);
    }

    @Override
    public void handleMessage(final String event, final GoannaBundle message, final EventCallback callback) {
        Log.i(LOG_TAG, "Handling event: " + event);
        ThreadUtils.assertOnBackgroundThread();

        final Context context = GoannaAppShell.getApplicationContext();
        // We're invoked in response to a Goanna message on a background thread.  We should always
        // be able to safely retrieve the current Goanna profile.
        final GoannaProfile goannaProfile = GoannaProfile.get(context);
        ...
        try {
            if ("PushServiceAndroidGCM:Initialized".equals(event)) {
                processComponentState(GoannaComponent.PushServiceAndroidGCM, true);
                callback.sendSuccess(null);
                return;
            }
            if ("PushServiceAndroidGCM:Uninitialized".equals(event)) {
                processComponentState(GoannaComponent.PushServiceAndroidGCM, false);
                callback.sendSuccess(null);
                return;
            }
            if ("FxAccountsPush:Initialized".equals(event)) {
                processComponentState(GoannaComponent.FxAccountsPush, true);
                callback.sendSuccess(null);
                return;
            }
            if ("PushServiceAndroidGCM:Configure".equals(event)) {
                ...
                pushManager.configure(goannaProfile.getName(), endpoint, debug, System.currentTimeMillis()); // For side effects.
                callback.sendSuccess(null);
                return;
            }
            ...
            if ("PushServiceAndroidGCM:DumpSubscriptions".equals(event)) {
                try {
                    final Map<String, PushSubscription> result = pushManager.allSubscriptionsForProfile(goannaProfile.getName());
            ...
            if ("PushServiceAndroidGCM:RegisterUserAgent".equals(event)) {
                try {
                    pushManager.registerUserAgent(goannaProfile.getName(), System.currentTimeMillis()); // For side-effects.
                    callback.sendSuccess(null);
            ...
            if ("PushServiceAndroidGCM:SubscribeChannel".equals(event)) {
                ...
                try {
                    ...
                    serviceData.put("profileName", goannaProfile.getName());
                    serviceData.put("profilePath", goannaProfile.getDir().getAbsolutePath());
                } catch (JSONException e) {
                    Log.e(LOG_TAG, "Got exception in " + event, e);
                    callback.sendError("Got exception handling message [" + event + "]: " + e.toString());
                    return;
                ...
                try {
                    subscription = pushManager.subscribeChannel(goannaProfile.getName(), service, serviceData, appServerKey, System.currentTimeMillis());
                } catch (PushManager.ProfileNeedsConfigurationException | AutopushClientException | PushClient.LocalException | IOException e) {
                    Log.e(LOG_TAG, "Got exception in " + event, e);
                    callback.sendError("Got exception handling message [" + event + "]: " + e.toString());
                    return;
            ...
            if ("History:GetPrePathLastVisitedTimeMilliseconds".equals(event)) {
                ...
                final long millis = BrowserDB.from(goannaProfile).getPrePathLastVisitedTimeMilliseconds(
    ...
    private void processComponentState(@NonNull GoannaComponent component, boolean isReady) {
        if (component == GoannaComponent.FxAccountsPush) {
            isReadyFxAccountsPush = isReady;

        } else if (component == GoannaComponent.PushServiceAndroidGCM) {
            isReadyPushServiceAndroidGCM = isReady;
        }

        // Send all pending messages to Goanna.
        if (canSendPushMessagesToGoanna()) {
            sendPushMessagesToGoanna(pendingPushMessages);
            pendingPushMessages.clear();
        }
    }

    private boolean canSendPushMessagesToGoanna() {
        return isReadyFxAccountsPush && isReadyPushServiceAndroidGCM;
    }

    private static void sendPushMessagesToGoanna(@NonNull List<JSONObject> messages) {
        for (JSONObject pushMessage : messages) {
            ...
            if (profileName == null || profilePath == null ||
                    !GoannaThread.canUseProfile(profileName, new File(profilePath))) {
            ...
            if (SERVICE_WEBPUSH.equals(service)) {
                sendMessageToGoannaService(pushMessage);
            } else if (SERVICE_FXA.equals(service)) {
                sendMessageToDecodeToGoannaService(pushMessage);
            }
        }
    }
}
