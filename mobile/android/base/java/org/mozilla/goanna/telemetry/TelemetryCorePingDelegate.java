...
import org.mozilla.goanna.adjust.AttributionHelperListener;
import org.mozilla.goanna.telemetry.measurements.CampaignIdMeasurements;
import org.mozilla.goanna.delegates.BrowserAppDelegateWithReference;
...
import org.mozilla.goanna.telemetry.measurements.SessionMeasurements;
import org.mozilla.goanna.telemetry.pingbuilders.TelemetryCorePingBuilder;
...
public class TelemetryCorePingDelegate extends BrowserAppDelegateWithReference
    ...
    private static final String LOGTAG = StringUtils.safeSubstring(
            "Goanna" + TelemetryCorePingDelegate.class.getSimpleName(), 0, 23);
    ...
    private TelemetryDispatcher getTelemetryDispatcher(final BrowserApp browserApp) {
        if (telemetryDispatcher == null) {
            final GoannaProfile profile = browserApp.getProfile();
    ...
    private SharedPreferences getSharedPreferences(final BrowserApp activity) {
        return GoannaSharedPrefs.forProfileName(activity, activity.getProfile().getName());
    }

    // via SearchEngineCallback - may be called from any thread.
    @Override
    public void execute(@Nullable final org.mozilla.goanna.search.SearchEngine engine) {
        ...
        ThreadUtils.postToBackgroundThread(new Runnable() {
            ...
            public void run() {
                ...
                final GoannaProfile profile = activity.getProfile();
