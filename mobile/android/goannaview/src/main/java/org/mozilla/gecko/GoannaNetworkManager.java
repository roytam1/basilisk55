...
import org.mozilla.goanna.util.NetworkUtils;
import org.mozilla.goanna.util.NetworkUtils.ConnectionSubType;
import org.mozilla.goanna.util.NetworkUtils.ConnectionType;
import org.mozilla.goanna.util.NetworkUtils.NetworkStatus;
 ...
 * This class depends on access to the context, so only use after GoannaAppShell has been initialized.
 */
public class GoannaNetworkManager extends BroadcastReceiver implements BundleEventListener {
    private static final String LOGTAG = "GoannaNetworkManager";

    private static final String LINK_DATA_CHANGED = "changed";

    private static GoannaNetworkManager instance;
    ...
    public static GoannaNetworkManager getInstance() {
        if (instance == null) {
            instance = new GoannaNetworkManager();
        }
    ...
    public double[] getCurrentInformation() {
        final Context applicationContext = GoannaAppShell.getApplicationContext();
    ...
    private synchronized boolean handleManagerEvent(ManagerEvent event) {
        ...
        // GoannaAppShell.getApplicationContext() will be null, and .start() wasn't called yet,
        ...
        if (context != null) {
            ...
            contextForAction = GoannaAppShell.getApplicationContext();
        }
    ...
    private void sendNetworkStateToListeners(final Context context) {
        if (currentConnectionType != previousConnectionType ||
            ...
            if (GoannaThread.isRunning()) {
                ...
                GoannaThread.queueNativeCall(GoannaNetworkManager.class, "onConnectionChanged",
        ...
        if (GoannaThread.isRunning()) {
            ...
            GoannaThread.queueNativeCall(GoannaNetworkManager.class, "onStatusChanged",
                                        String.class, status);
        }
    }
    ...
    private void getWifiIPAddress(final EventCallback callback) {
        final WifiManager mgr = (WifiManager) GoannaAppShell.getApplicationContext().getSystemService(Context.WIFI_SERVICE);
    ...
    public static int getMCC() {
        return getNetworkOperator(InfoType.MCC, GoannaAppShell.getApplicationContext());
    }

    @JNITarget
    public static int getMNC() {
        return getNetworkOperator(InfoType.MNC, GoannaAppShell.getApplicationContext());
    }
}
