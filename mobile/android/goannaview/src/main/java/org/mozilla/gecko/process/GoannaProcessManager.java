...
public final class GoannaProcessManager {
    private static final String LOGTAG = "GoannaProcessManager";
    private static final GoannaProcessManager INSTANCE = new GoannaProcessManager();

    public static GoannaProcessManager getInstance() {
        return INSTANCE;
    }
    ...
    private static final class ChildConnection implements ServiceConnection, IBinder.DeathRecipient {
        ...
        public void binderDied() {
            ...
            try {
                GoannaAppShell.getApplicationContext().unbindService(this);
    ...
    public int start(String type, String[] args, int crashFd, int ipcFd) {
        ...
        try {
            ...
            intent.setClassName(GoannaAppShell.getApplicationContext(),
                                "org.mozilla.goanna.process.GoannaServiceChildProcess$" + type);
            GoannaLoader.addEnvironmentToIntent(intent);
            connection.prepareToWait();
            GoannaAppShell.getApplicationContext().bindService(intent, connection, Context.BIND_AUTO_CREATE);
            connection.waitForChild();
            if (connection.mChild == null) {
                // FAILED TO CONNECT.
                Log.e(LOGTAG, "Failed to connect to child process of '" + type + "'");
                GoannaAppShell.getApplicationContext().unbindService(connection);
                return 0;
            }
