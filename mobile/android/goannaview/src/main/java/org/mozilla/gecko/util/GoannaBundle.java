 ...
 */
@RobocopTarget
public final class GoannaBundle {
    private static final String LOGTAG = "GoannaBundle";
    ...
    private static final GoannaBundle[] EMPTY_BUNDLE_ARRAY = new GoannaBundle[0];
    ...
    /**
     * Construct an empty GoannaBundle.
     */
    public GoannaBundle() {
    ...
    /**
     * Construct an empty GoannaBundle with specific capacity.
     *
     * @param capacity Initial capacity.
     */
    public GoannaBundle(final int capacity) {
        mMap = new SimpleArrayMap<>(capacity);
    }

    /**
     * Construct a copy of another GoannaBundle.
     *
     * @param bundle GoannaBundle to copy from.
     */
    public GoannaBundle(final GoannaBundle bundle) {
        mMap = new SimpleArrayMap<>(bundle.mMap);
    }

    @WrapForJNI(calledFrom = "goanna")
    private GoannaBundle(final String[] keys, final Object[] values) {
    ...
    // The only case where we convert String[] to/from GoannaBundle[] is if every element
    // is null.
    ...
    /**
     * Returns the value associated with a GoannaBundle mapping, or null if the mapping
     * does not exist.
     *
     * @param key Key to look for.
     * @return GoannaBundle value
     */
    public GoannaBundle getBundle(final String key) {
        return (GoannaBundle) mMap.get(key);
    }

    /**
     * Returns the value associated with a GoannaBundle array mapping, or null if the
     * mapping does not exist.
     *
     * @param key Key to look for.
     * @return GoannaBundle array value
     */
    public GoannaBundle[] getBundleArray(final String key) {
        final Object value = mMap.get(key);
        return value == null ? null : Array.getLength(value) == 0 ? EMPTY_BUNDLE_ARRAY :
               !(value instanceof GoannaBundle[]) ? new GoannaBundle[getNullArrayLength(value)] :
                                                   (GoannaBundle[]) value;
    }

    /**
     * Returns whether this GoannaBundle has no mappings.
     *
    ...
    /**
     * Map a key to a GoannaBundle value.
     *
     * @param key Key to map.
     * @param value Value to map to.
     */
    public void putBundle(final String key, final GoannaBundle value) {
        mMap.put(key, value);
    }

    /**
     * Map a key to a GoannaBundle array value.
     *
     * @param key Key to map.
     * @param value Value to map to.
     */
    public void putBundleArray(final String key, final GoannaBundle[] value) {
        mMap.put(key, value);
    }
     ...
     * @param value Value to map to.
     */
    public void putBundleArray(final String key, final Collection<GoannaBundle> value) {
        if (value == null) {
            mMap.put(key, null);
            return;
        }
        final GoannaBundle[] array = new GoannaBundle[value.size()];
        int i = 0;
        for (final GoannaBundle element : value) {
            array[i++] = element;
        }
        mMap.put(key, array);
    }
    ...
    /**
     * Returns number of mappings in this GoannaBundle.
     *
    ...
    public boolean equals(Object other) {
        if (!(other instanceof GoannaBundle)) {
            return false;
        }

        // Support library's SimpleArrayMap.equals is buggy, so roll our own version.
        final SimpleArrayMap<String, Object> otherMap = ((GoannaBundle) other).mMap;
        if (mMap == otherMap) {
            return true;
        }
    ...
    public JSONObject toJSONObject() throws JSONException {
        ...
        for (int i = 0; i < mMap.size(); i++) {
            ...
            if (value instanceof GoannaBundle) {
                jsonValue = ((GoannaBundle) value).toJSONObject();
            } else if (value instanceof GoannaBundle[]) {
                final GoannaBundle[] array = (GoannaBundle[]) value;
    ...
    public Bundle toBundle() {
        ...
        for (int i = 0; i < mMap.size(); i++) {
            ...
            } else if (val instanceof GoannaBundle) {
                out.putBundle(key, ((GoannaBundle) val).toBundle());
            } else if (val instanceof GoannaBundle[]) {
                final GoannaBundle[] array = (GoannaBundle[]) val;
    ...
    public static GoannaBundle fromBundle(final Bundle bundle) {
        if (bundle == null) {
            return null;
        }
        ...
        for (final String key : bundle.keySet()) {
            ...
            if (value instanceof Bundle || value == null) {
                ...
                final GoannaBundle[] out = new GoannaBundle[array.length];
                for (int j = 0; j < array.length; j++) {
        ...
        return new GoannaBundle(keys, values);
    }
    ...
    public static GoannaBundle fromJSONObject(final JSONObject obj) throws JSONException {
