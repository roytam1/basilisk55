...
class GoannaInstance(object):
    ...
    def __init__(self, host=None, port=None, bin=None, profile=None, addons=None,
                 app_args=None, symbols_path=None, goanna_log=None, prefs=None,
        ...
        self._goanna_log_option = goanna_log
        self._goanna_log = None
        self.verbose = verbose

    @property
    def goanna_log(self):
        if self._goanna_log:
            return self._goanna_log

        path = self._goanna_log_option
        if path != "-":
            if path is None:
                path = "goanna.log"
            elif os.path.isdir(path):
                fname = "goanna-{}.log".format(time.time())
...
class FennecInstance(GoannaInstance):
    fennec_prefs = {
    ...
    def start(self):
        ...
        # goanna_log comes from logcat when running with device/emulator
        logcat_args = {
            "filterspec": "Goanna",
        ...
        if self.goanna_log == "-":
            ...
            logcat_args["logfile"] = self.goanna_log
...
class DesktopInstance(GoannaInstance):
    desktop_prefs = {
