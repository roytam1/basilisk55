...
//static
auto
GoannaChildProcessHost::GetPathToBinary(FilePath& exePath, GoannaProcessType processType) -> BinaryPathType
{
  if (sRunSelfAsContentProc &&
      (processType == GoannaProcessType_Content || processType == GoannaProcessType_GPU)) {
#if defined(OS_WIN)
...
uint32_t GoannaChildProcessHost::sNextUniqueID = 1;

/* static */
uint32_t
GoannaChildProcessHost::GetUniqueID()
{
...
GoannaChildProcessHost::SyncLaunch(std::vector<std::string> aExtraOpts, int aTimeoutMs, base::ProcessArchitecture arch)
{
  ...
  ioLoop->PostTask(NewNonOwningRunnableMethod
                   ...
                   (this, &GoannaChildProcessHost::RunPerformAsyncLaunch,
                    aExtraOpts, arch));
...
void
GoannaChildProcessHost::SetChildLogName(const char* varName, const char* origLogName,
                                       nsACString &buffer)
{
...
//--------------------------------------------------
...
#endif
  {
    ...
    // child processes.
    if (mProcessType == GoannaProcessType_Content ||
        mProcessType == GoannaProcessType_GPU ||
        mProcessType == GoannaProcessType_GMPlugin) {
...
void
GoannaChildProcessHost::OnMessageReceived(IPC::Message&& aMsg)
{
...
bool GoannaChildProcessHost::sRunSelfAsContentProc(false);

#ifdef MOZ_WIDGET_ANDROID
void
GoannaChildProcessHost::LaunchAndroidService(const char* type,
                                            ...
                                            ProcessHandle* process_handle)
{
  ...
  int32_t handle = java::GoannaAppShell::StartGoannaServiceChildProcess(type, jargs, crashFd, ipcFd);
