...
public class ZoomedView extends FrameLayout implements LayerView.DynamicToolbarListener,
    ...
    private enum StartPointUpdate {
        GOANNA_POSITION, CENTER, NO_CHANGE
    }
            ...
            case MotionEvent.ACTION_UP:
                if (dragged) {
                    ...
                    if (isClickInZoomedView(event.getY())) {
                        GoannaAppShell.notifyObservers("Gesture:ClickInZoomedView", "");
    ...
    private void moveZoomedView(ImmutableViewportMetrics metrics, float newLeftMargin, float newTopMargin,
        ...
        if (animateStartPoint == StartPointUpdate.GOANNA_POSITION) {
            // Before this point, the animationStart point is relative to the layerView.
            // The value is initialized in startZoomDisplay using the click point position coming from Goanna.
    ...
    private void startZoomDisplay(LayerView aLayerView, final int leftFromGoanna, final int topFromGoanna) {
        ...
        animationStart.x = (float) leftFromGoanna * metrics.zoomFactor;
        animationStart.y = (float) topFromGoanna * metrics.zoomFactor + layerView.getSurfaceTranslation();

        moveUsingGoannaPosition(leftFromGoanna, topFromGoanna);
    }

    public void stopZoomDisplay(boolean withAnimation) {
        // If "startZoomDisplay" is running and not totally completed (Goanna thread is still
    ...
    public void handleMessage(final String event, final GoannaBundle message,
        ...
        if ("Gesture:CloseZoomedView".equals(event)) {
            ...
            final GoannaBundle clickPosition = message.getBundle("clickPosition");
            final int left = clickPosition.getInt("x");
            final int top = clickPosition.getInt("y");
            // Start to display inside the zoomedView
            final LayerView goannaAppLayerView = GoannaAppShell.getLayerView();
    ...
    private void moveUsingGoannaPosition(int leftFromGoanna, int topFromGoanna) {
        ...
        moveToolbar((topFromGoanna * metrics.zoomFactor > parentHeight / 2));
        ...
        moveZoomedView(metrics, convertedPosition.x, convertedPosition.y, StartPointUpdate.GOANNA_POSITION);
    }
