...
final class LocalMediaDrmBridge implements GoannaMediaDrm {
    private static final String LOGTAG = "GoannaLocalMediaDrmBridge";
    private static final boolean DEBUG = false;
    private GoannaMediaDrm mBridge = null;
    private CallbacksForwarder mCallbacksFwd;

    // Forward the callback calls from GoannaMediaDrmBridgeV{21,23}
    // to the callback MediaDrmProxy.Callbacks.
    private class CallbacksForwarder implements GoannaMediaDrm.Callbacks {
        private final GoannaMediaDrm.Callbacks mProxyCallbacks;

        CallbacksForwarder(GoannaMediaDrm.Callbacks callbacks) {
    ...
    LocalMediaDrmBridge(String keySystem) throws Exception {
        if (AppConstants.Versions.preLollipop) {
            ...
            mBridge = new GoannaMediaDrmBridgeV21(keySystem);
        } else {
            mBridge = new GoannaMediaDrmBridgeV23(keySystem);
        }
    }
