...
class GoannaLayerClient implements LayerView.Listener, PanZoomTarget
{
    ...
    private volatile boolean mGoannaIsReady;
    ...
    public GoannaLayerClient(Context context, LayerView view) {
    ...
    public void setGoannaReady(boolean ready) {
        mGoannaIsReady = ready;
    }

    @Override // PanZoomTarget
    public boolean isGoannaReady() {
        return mGoannaIsReady;
    }

    /** Attaches to root layer so that Goanna appears. */
    @WrapForJNI(calledFrom = "goanna")
    private void onGoannaReady() {
        mGoannaIsReady = true;
    ...
    private void sendResizeEventIfNecessary(boolean force, PointF scrollChange) {
        ...
        GoannaAppShell.notifyObservers("Window:Resize", json);
    }
    ...
    Matrix getMatrixForLayerRectToViewRect() {
        ...
        ImmutableViewportMetrics goannaViewport = mViewportMetrics;
        PointF goannaOrigin = goannaViewport.getOrigin();
        float goannaZoom = goannaViewport.zoomFactor;

        Matrix matrix = new Matrix();
        matrix.postTranslate(goannaOrigin.x / goannaZoom, goannaOrigin.y / goannaZoom);
