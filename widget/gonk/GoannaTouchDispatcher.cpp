...
/* static */ GoannaTouchDispatcher*
GoannaTouchDispatcher::GetInstance()
{
  if (!sTouchDispatcher) {
    sTouchDispatcher = new GoannaTouchDispatcher();
...
void
GoannaTouchDispatcher::SetCompositorVsyncScheduler(mozilla::layers::CompositorVsyncScheduler *aObserver)
{
  MOZ_ASSERT(NS_IsMainThread());
...
GoannaTouchDispatcher::NotifyTouch(MultiTouchInput& aTouch, TimeStamp aEventTime)
{
  ...
  if (aTouch.mType == MultiTouchInput::MULTITOUCH_MOVE) {
    ...
    layers::APZThreadUtils::RunOnControllerThread(NewRunnableMethod<MultiTouchInput>(
      this, &GoannaTouchDispatcher::DispatchTouchNonMoveEvent, aTouch));
  }
}

void
GoannaTouchDispatcher::DispatchTouchNonMoveEvent(MultiTouchInput aInput)
{
  layers::APZThreadUtils::AssertOnControllerThread();
