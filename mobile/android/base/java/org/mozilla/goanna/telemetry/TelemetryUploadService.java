...
import org.mozilla.goanna.telemetry.stores.TelemetryPingStore;
import org.mozilla.goanna.util.DateUtil;
...
public class TelemetryUploadService extends IntentService {
    private static final String LOGTAG = StringUtils.safeSubstring("Goanna" + TelemetryUploadService.class.getSimpleName(), 0, 23);
     ...
     * {@link #isUploadEnabledByProfileConfig(Context, GoannaProfile)} if the profile is available as it takes into
    ...
    public static boolean isUploadEnabledByAppConfig(final Context context) {
        ...
        if (!GoannaPreferences.getBooleanPref(context, GoannaPreferences.PREFS_HEALTHREPORT_UPLOAD_ENABLED, true)) {
     ...
     * Note that this method logs debug statements when upload is disabled.
     */
    public static boolean isUploadEnabledByProfileConfig(final Context context, final GoannaProfile profile) {
