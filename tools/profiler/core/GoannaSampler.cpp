...
GoannaSampler::GoannaSampler(double aInterval, int aEntrySize,
...
void GoannaSampler::HandleSaveRequest()
{
  if (!mSaveRequested)
    return;
  mSaveRequested = false;
...
void GoannaSampler::DeleteExpiredMarkers()
{
  mBuffer->deleteExpiredStoredMarkers();
}

void GoannaSampler::StreamTaskTracer(SpliceableJSONWriter& aWriter)
{
#ifdef MOZ_TASK_TRACER
...
void GoannaSampler::StreamMetaJSCustomObject(SpliceableJSONWriter& aWriter)
{
...
void GoannaSampler::ToStreamAsJSON(std::ostream& stream, double aSinceTime)
{
  SpliceableJSONWriter b(mozilla::MakeUnique<OStreamJSONWriteFunc>(stream));
  StreamJSON(b, aSinceTime);
}

JSObject* GoannaSampler::ToJSObject(JSContext *aCx, double aSinceTime)
{
...
void GoannaSampler::GetGatherer(nsISupports** aRetVal)
{
...
UniquePtr<char[]> GoannaSampler::ToJSON(double aSinceTime)
{
...
void GoannaSampler::ToJSObjectAsync(double aSinceTime,
...
void GoannaSampler::ToFileAsync(const nsACString& aFileName, double aSinceTime)
{
  if (NS_WARN_IF(!mGatherer)) {
    return;
  }
    ...
    for (int sampleId = 0; true; sampleId++) {
      ...
      for (int frameId = 0; true; frameId++) {
        jni::String::LocalRef frameName =
            java::GoannaJavaSampler::GetFrameName(0, sampleId, frameId);
        ...
        if (firstRun) {
          ...
          double sampleTime =
              java::GoannaJavaSampler::GetSampleTime(0, sampleId);
...
void GoannaSampler::StreamJSON(SpliceableJSONWriter& aWriter, double aSinceTime)
{
  aWriter.Start(SpliceableJSONWriter::SingleLineStyle);
  {
    ...
    aWriter.StartArrayProperty("threads");
    {
      ...
      if (ProfileJava()) {
        ...
        java::GoannaJavaSampler::Unpause();
      }
  #endif
...
void GoannaSampler::FlushOnJSShutdown(JSContext* aContext)
{
  SetPaused(true);
...
void GoannaSampler::doNativeBacktrace(ThreadProfile &aProfile, TickSample* aSample)
{
  void* pc_array[1000];
  void* sp_array[1000];
...
void GoannaSampler::Tick(TickSample* sample)
{
  // Don't allow for ticks to happen within other ticks.
  InplaceTick(sample);
}

void GoannaSampler::InplaceTick(TickSample* sample)
{
...
SyncProfile* GoannaSampler::GetBacktrace()
{
...
void
GoannaSampler::GetBufferInfo(uint32_t *aCurrentPosition, uint32_t *aTotalSize, uint32_t *aGeneration)
{
