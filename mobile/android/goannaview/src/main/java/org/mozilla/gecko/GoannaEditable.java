...
*/
final class GoannaEditable extends JNIObject
        implements InvocationHandler, Editable, GoannaEditableClient {
    ...
    /* package */ GoannaEditableListener mListener;
    /* package */ GoannaView mView;

    /* package */ boolean mInBatchMode; // Used by IC thread
    /* package */ boolean mNeedSync; // Used by IC thread
    // Goanna side needs an updated composition from Java;
    ...
    private boolean mIgnoreSelectionChange; // Used by Goanna thread
    ...
    private void onKeyEvent(KeyEvent event, int action, int savedMetaState,
        ...
        onKeyEvent(action, event.getKeyCode(), event.getScanCode(),
                   ...
                   // but Goanna expects 'a', so we return that in baseUnicodeChar.
     ...
     * modified on the Goanna side and reflects the authoritative version of the text. The
    ...
    private final class AsyncText {
        ...
        public synchronized void syncShadowText(final GoannaEditableListener listener) {
            if (DEBUG) {
                assertOnIcThread();
            }
    ...
    */
    ...
    private void icOfferAction(final Action action) {
        ...
        case Action.TYPE_REPLACE_TEXT:
            // Always sync text after a replace action, so that if the Goanna
            ...
            if (!icMaybeSendComposition(
                    action.mSequence, /* useEntireText */ true, /* notifyGoanna */ false)) {
    ...
    GoannaEditable(final GoannaView v) {
        if (DEBUG) {
            // Called by nsWindow.
            ThreadUtils.assertOnGoannaThread();
        }
    ...
    @WrapForJNI(calledFrom = "goanna")
    private void onViewChange(final GoannaView v) {
        if (DEBUG) {
            // Called by nsWindow.
            ThreadUtils.assertOnGoannaThread();
        ...
        final GoannaEditableListener newListener =
            v != null ? GoannaInputConnection.create(v, this) : null;
        ...
        final Runnable setListenerRunnable = new Runnable() {
            ...
            public void run() {
                ...
                if (newListener == null) {
                    ...
                    // disposeNative here.
                    GoannaEditable.this.disposeNative();
                }
            }
        };
    ...
    /**
     * Send composition ranges to Goanna for the entire shadow text.
     */
    private void icMaybeSendComposition() {
        ...
        icMaybeSendComposition(mText.getShadowText(),
                               /* useEntireText */ false, /* notifyGoanna */ true);
    }

    /**
     * Send composition ranges to Goanna if the text has composing spans.
     *
     ...
     *                      a Goanna composition, instead of just the spanned part.
     * @param notifyGoanna Notify Goanna of the new composition ranges;
     *                    otherwise, the caller is responsible for notifying Goanna.
    ...
    private void goannaScheduleSyncShadowText() {
        if (DEBUG) {
            ThreadUtils.assertOnGoannaThread();
        }
    ...
    private void goannaActionReply(final Action action) {
        if (!mGoannaFocused) {
            if (DEBUG) {
    ...
    private void notifyCommitComposition() {
        // Goanna already committed its composition. However, Android keyboards
    ...
    private void notifyIME(final int type) {
        if (DEBUG) {
            ...
            if (type != GoannaEditableListener.NOTIFY_IME_REPLY_EVENT) {
        ...
        if (type == GoannaEditableListener.NOTIFY_IME_REPLY_EVENT) {
            goannaActionReply(mActions.poll());
            if (!mGoannaFocused || !mActions.isEmpty()) {
                // Only post to IC thread below when the queue is empty.
                return;
            }
        } else if (type == GoannaEditableListener.NOTIFY_IME_TO_COMMIT_COMPOSITION) {
            notifyCommitComposition();
            return;
        } else if (type == GoannaEditableListener.NOTIFY_IME_TO_CANCEL_COMPOSITION) {
        ...
        goannaPostToIc(new Runnable() {
            ...
            public void run() {
                ...
                if (type == GoannaEditableListener.NOTIFY_IME_OF_FOCUS && mListener != null) {
                    mNeedSync = false;
        ...
        // Update the mGoannaFocused flag.
        if (type == GoannaEditableListener.NOTIFY_IME_OF_BLUR) {
            mGoannaFocused = false;
        } else if (type == GoannaEditableListener.NOTIFY_IME_OF_FOCUS) {
            mGoannaFocused = true;
        }
    }
    ...
    @WrapForJNI(calledFrom = "goanna", exceptionMode = "ignore")
    private void onSelectionChange(final int start, final int end) {
        ...
        goannaScheduleSyncShadowText();
    }

    private boolean goannaIsSameText(int start, int oldEnd, CharSequence newText) {
    ...
    private void onTextChange(final CharSequence text, final int start,
        ...
        if (start == 0 && unboundedOldEnd > currentLength) {
            ...
            // Don't ignore the next selection change because we are re-syncing with Goanna
            mIgnoreSelectionChange = false;
        ...
        } else if (goannaIsSameText(start, oldEnd, text)) {
            ...
            // Goanna side initiated the text change. Replace in two steps to properly
