...
abstract class BaseTest extends BaseRobocopTest {
    ...
    private static final int GOANNA_READY_WAIT_MS = 180000;
    ...
    protected void blockForDelayedStartup() {
        try {
            Actions.EventExpecter delayedStartupExpector =
                    mActions.expectGlobalEvent(Actions.EventType.UI, "Goanna:DelayedStartup");
            delayedStartupExpector.blockForEvent(GOANNA_READY_WAIT_MS, true);
    ...
    protected void blockForGoannaReady() {
        try {
            Actions.EventExpecter goannaReadyExpector =
                    mActions.expectGlobalEvent(Actions.EventType.GOANNA, "Goanna:Ready");
            if (!GoannaThread.isRunning()) {
                goannaReadyExpector.blockForEvent(GOANNA_READY_WAIT_MS, true);
            }
    ...
    protected final void hitEnterAndWait() {
        Actions.EventExpecter contentEventExpecter =
                mActions.expectGlobalEvent(Actions.EventType.GOANNA, "Content:DOMContentLoaded");
    ...
    /**
     * Load <code>url</code> using the internal
     * <code>org.mozilla.goanna.Tabs</code> API and wait for DOMContentLoaded.
     */
    ...
    public void addTab() {
        ...
        for(;;) {
            final GoannaBundle data = pageShowExpecter.blockForBundle();
