...
class GoannaInputConnection
    ...
    implements InputConnectionListener, GoannaEditableListener {
    ...
    private void showSoftInput() {
        ...
        v.post(new Runnable() {
            ...
            public void run() {
                ...
                GoannaAppShell.getLayerView().getDynamicToolbarAnimator().showToolbar(/*immediately*/true);
    ...
    @Override // GoannaEditableListener
    ...
    public void updateCompositionRects(final RectF[] aRects) {
        ...
        // Calculate Goanna logical coords to screen coords
        final View v = getView();
        if (v == null) {
            return;
        }
        ...
        DynamicToolbarAnimator animator = GoannaAppShell.getLayerView().getDynamicToolbarAnimator();
        float toolbarHeight = animator.getMaxTranslation() - animator.getToolbarTranslation();

        Matrix matrix = GoannaAppShell.getLayerView().getMatrixForLayerRectToViewRect();
        if (matrix == null) {
            if (DEBUG) {
                Log.d(LOGTAG, "Cannot get Matrix to convert from Goanna coords to layer view coords");
            }
            return;
        }
    ...
    public boolean requestCursorUpdates(int cursorUpdateMode) {
        ...
        if ((cursorUpdateMode & InputConnection.CURSOR_UPDATE_IMMEDIATE) != 0) {
            mEditableClient.requestCursorUpdates(GoannaEditableClient.ONE_SHOT);
        }

        if ((cursorUpdateMode & InputConnection.CURSOR_UPDATE_MONITOR) != 0) {
            mEditableClient.requestCursorUpdates(GoannaEditableClient.START_MONITOR);
        } else {
            mEditableClient.requestCursorUpdates(GoannaEditableClient.END_MONITOR);
        }
        return true;
    }
    ...
    public void onDefaultKeyEvent(final KeyEvent event) {
        ThreadUtils.postToUiThread(new Runnable() {
            @Override
            public void run() {
                GoannaInputConnection.this.performDefaultKeyAction(event);
            }
        });
    }
    ...
    // Called by OnDefaultKeyEvent handler, up from Goanna
