...
public final class NotificationHelper implements BundleEventListener {
    ...
    private SimpleArrayMap<String, GoannaBundle> mClearableNotifications;
    ...
    public static void getArgsAndSendNotificationIntent(SafeIntent intent) {
        ...
        try {
            ...
            GoannaAppShell.notifyObservers("Notification:Event", args.toString());
    ...
    private Uri.Builder getNotificationBuilder(final GoannaBundle message, final String type) {
    ...
    private Intent buildNotificationIntent(final GoannaBundle message, final Uri.Builder builder) {
        ...
        // All intents get routed through the notificationReceiver. That lets us bail if we don't want to start Goanna
        final ComponentName name = new ComponentName(
                mContext, GoannaAppShell.getGoannaInterface().getActivity().getClass());
        notificationIntent.putExtra(ORIGINAL_EXTRA_COMPONENT, name);

        return notificationIntent;
    }

    private PendingIntent buildNotificationPendingIntent(final GoannaBundle message,
                                                         final String type) {
    ...
    private PendingIntent buildButtonClickPendingIntent(final GoannaBundle message,
                                                        final GoannaBundle action) {
    ...
    private void showNotification(final GoannaBundle message) {
        ...
        final GoannaBundle[] actions = message.getBundleArray(ACTIONS_ATTR);
        if (actions != null) {
            for (int i = 0; i < actions.length; i++) {
                final GoannaBundle action = actions[i];
        ...
        // If the goanna event is for download completion, we create another intent with different
        ...
        ((NotificationClient) GoannaAppShell.getNotificationListener()).add(id, builder.build());
    ...
    private Intent createIntentIfDownloadCompleted(final GoannaBundle message) {
    ...
    private void hideNotification(final GoannaBundle message) {
    ...
    private void closeNotification(String id, String handlerKey, String cookie) {
        ((NotificationClient) GoannaAppShell.getNotificationListener()).remove(id);
    }
    ...
    private void clearAll() {
        ...
        for (int i = 0; i < mClearableNotifications.size(); i++) {
            ...
            final GoannaBundle obj = mClearableNotifications.valueAt(i);
