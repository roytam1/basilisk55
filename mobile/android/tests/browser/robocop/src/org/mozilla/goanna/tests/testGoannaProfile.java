...
public class testGoannaProfile extends PixelTest {
    ...
    private void checkDefaultGetter() {
        ...
        mAsserter.info("Test using the test profile", GoannaProfile.CUSTOM_PROFILE);
        GoannaProfile profile = GoannaProfile.get(getActivity());
        verifyProfile(profile, GoannaProfile.CUSTOM_PROFILE, ((GoannaApp) getActivity()).getProfile().getDir(), true);
    ...
    private void checkNamedGetter(String name) {
        ...
        if (name != null) {
            ...
            verifyProfile(profile, GoannaProfile.CUSTOM_PROFILE, defaultProfile, true);
        }
    }
    ...
    private void checkNameAndDirGetter(final String name, final boolean useFile,
        ...
        final File testProfileDir = ((GoannaApp) getActivity()).getProfile().getDir();
        final String expectedName = name != null ? name : GoannaProfile.CUSTOM_PROFILE;

        final GoannaProfile profile;
        if (useFile) {
            profile = GoannaProfile.get(getActivity(), name, file);
        } else {
            profile = GoannaProfile.get(getActivity(), name, path);
        }

        if (name != null || f != null) {
            // GoannaProfile will create a directory and add an ini section if f is null
            ...
            if (name == null) {
                // A side effect of calling GoannaProfile.get with null name is it changes
                // the test profile's directory to the new directory. Restore it back.
                GoannaProfile.get(getActivity(), null, testProfileDir);
                mAsserter.is(GoannaProfile.get(getActivity()).getDir(), testProfileDir,
    ...
    private void checkGuestProfile() {
        ...
        GoannaProfile profile = GoannaProfile.getGuestProfile(getActivity());
        verifyProfile(profile, GoannaProfile.CUSTOM_PROFILE, getActivity().getFileStreamPath("guest"), true);
        ...
        mAsserter.ok(GoannaProfile.removeProfile(getActivity(), profile), "Cleaned up unlocked guest profile", profile.getName());
        mAsserter.ok(!dir.exists(), "Guest dir was deleted", dir.toString());

        // Restore test profile directory, which was changed in the last GoannaProfile.get call.
        GoannaProfile.get(getActivity(), null, testProfileDir);
        mAsserter.is(GoannaProfile.get(getActivity()).getDir(), testProfileDir,
                     "Test profile should be restored");
    }
    ...
    private void removeProfile(GoannaProfile profile, boolean inProfilesIni) {
        ...
        mAsserter.ok(GoannaProfile.removeProfile(getActivity(), profile), "Remove was successful", name);
