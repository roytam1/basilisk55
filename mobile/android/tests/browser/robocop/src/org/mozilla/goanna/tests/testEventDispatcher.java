...
public class testEventDispatcher extends JavascriptBridgeTest
        implements BundleEventListener, GoannaEventListener, NativeEventListener {

    private static final String TEST_JS = "testEventDispatcher.js";
    private static final String GOANNA_EVENT = "Robocop:TestGoannaEvent";
    private static final String GOANNA_RESPONSE_EVENT = "Robocop:TestGoannaResponse";
    ...
    private boolean handledGoannaEvent;
    ...
    public void setUp() throws Exception {
        ...
        EventDispatcher.getInstance().registerGoannaThreadListener(
                (GoannaEventListener) this, GOANNA_EVENT, GOANNA_RESPONSE_EVENT);
        EventDispatcher.getInstance().registerGoannaThreadListener(
    ...
    public void testEventDispatcher() {
        ...
        getJS().syncCall("send_test_message", GOANNA_EVENT);
        fAssertTrue("Should have handled Goanna event synchronously", handledGoannaEvent);

        getJS().syncCall("send_message_for_response", GOANNA_RESPONSE_EVENT, "success");
        getJS().syncCall("send_message_for_response", GOANNA_RESPONSE_EVENT, "error");
        ...
        // Test GoannaView-specific EventDispatcher.
        testScope("window");
    ...
    private static EventDispatcher getDispatcher(final String scope) {
        ...
        if ("window".equals(scope)) {
            return GoannaApp.getEventDispatcher();
        }
    ...
    private void testScope(final String scope) {
        ...
        // Test Goanna thread events in JS.
    ...
    public void handleMessage(final String event, final GoannaBundle message,
        ...
        if (UI_EVENT.equals(event) || BACKGROUND_EVENT.equals(event)) {
            ...
            final GoannaBundle[] objectArray = message.getBundleArray("objectArray");
            ...
            final GoannaBundle[] objectArrayOfNull = message.getBundleArray("objectArrayOfNull");
            ...
            final GoannaBundle[] emptyObjectArray = message.getBundleArray("emptyObjectArray");
    ...
    public void handleMessage(final String event, final JSONObject message) {
        ...
        try {
            if (GOANNA_EVENT.equals(event)) {
                checkJSONObject(message);
                checkJSONObject(message.getJSONObject("object"));
                handledGoannaEvent = true;

            } else if (GOANNA_RESPONSE_EVENT.equals(event)) {
                final String response = message.getString("response");
                if ("success".equals(response)) {
    ...
    private void checkBundle(final GoannaBundle bundle) {
        fAssertEquals("Bundle boolean has correct value", true, bundle.getBoolean("boolean"));
        fAssertEquals("Bundle int has correct value", 1, bundle.getInt("int"));
        fAssertEquals("Bundle double has correct value", 0.5, bundle.getDouble("double"));
        fAssertEquals("Bundle string has correct value", "foo", bundle.getString("string"));
    ...
    private static GoannaBundle createInnerBundle() {
        final GoannaBundle bundle = new GoannaBundle();
    ...
    private static GoannaBundle createBundle() {
        final GoannaBundle outer = createInnerBundle();
        final GoannaBundle inner = createInnerBundle();

        outer.putBundle("object", inner);
        outer.putBundle("nullObject", null);
        outer.putBundleArray("objectArray", new GoannaBundle[] {null, inner});
        outer.putBundleArray("objectArrayOfNull", new GoannaBundle[2]);
        outer.putBundleArray("emptyObjectArray", new GoannaBundle[0]);
        outer.putBundleArray("nullObjectArray", (GoannaBundle[]) null);
    ...
    public void dispatchMessageForResponse(final String scope, final String type,
        ...
        final GoannaBundle bundle = new GoannaBundle(1);
