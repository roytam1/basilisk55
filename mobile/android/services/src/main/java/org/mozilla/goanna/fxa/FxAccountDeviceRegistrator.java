...
import org.mozilla.goanna.background.fxa.FxAccountClient20.AccountStatusResponse;
import org.mozilla.goanna.background.fxa.FxAccountClient20.RequestDelegate;
import org.mozilla.goanna.background.fxa.FxAccountClientException.FxAccountClientRemoteException;
import org.mozilla.goanna.background.fxa.FxAccountRemoteError;
import org.mozilla.goanna.fxa.authenticator.AndroidFxAccount;
import org.mozilla.goanna.fxa.authenticator.AndroidFxAccount.InvalidFxAState;
...
public class FxAccountDeviceRegistrator implements BundleEventListener {
  ...
  private void beginRegistration(Context context) {
    // Fire up goanna and send event
    // We create the Intent ourselves instead of using GoannaService.getIntentToCreateServices
    // because we can't import these modules (circular dependency between browser and services)
    final Intent goannaIntent = buildCreatePushServiceIntent(context, "android-fxa-subscribe");
    context.startService(goannaIntent);
    // -> handleMessage()
  }
  ...
  private void beginRegistrationRenewal(Context context) {
    ...
    final Intent goannaIntent = buildCreatePushServiceIntent(context, "android-fxa-resubscribe");
    context.startService(goannaIntent);
    // -> handleMessage()
  }
  ...
  private Intent buildCreatePushServiceIntent(final Context context, final String data) {
    ...
    intent.setClassName(context, "org.mozilla.goanna.GoannaService");
    intent.putExtra("category", "android-push-service");
    intent.putExtra("data", data);
    final AndroidFxAccount fxAccount = AndroidFxAccount.fromContext(context);
    intent.putExtra("org.mozilla.goanna.intent.PROFILE_NAME", fxAccount.getProfile());
    return intent;
  }
  ...
  private void doFxaRegistration(GoannaBundle subscription) throws InvalidFxAState {
  ...
  private static void doFxaRegistration(final Context context, final GoannaBundle subscription, final boolean allowRecursion) throws InvalidFxAState {
  ...
  private static void recoverFromDeviceSessionConflict(final FxAccountClientRemoteException error,
                                                       ...
                                                       final GoannaBundle subscription,
  ...
  private void setupListeners() throws ClassNotFoundException, NoSuchMethodException,
    ...
    Class<?> eventDispatcher = Class.forName("org.mozilla.goanna.EventDispatcher");
