...
import org.mozilla.goanna.widget.ExternalIntentDuringPrivateBrowsingPromptFragment;
...
public final class IntentHelper implements GoannaEventListener,
    ...
    private IntentHelper(final FragmentActivity activity) {
        ...
        EventDispatcher.getInstance().registerGoannaThreadListener((GoannaEventListener) this, EVENTS);
        EventDispatcher.getInstance().registerGoannaThreadListener((NativeEventListener) this, NATIVE_EVENTS);
    }
    ...
    public static void destroy() {
        if (instance != null) {
            EventDispatcher.getInstance().unregisterGoannaThreadListener((GoannaEventListener) instance, EVENTS);
            EventDispatcher.getInstance().unregisterGoannaThreadListener((NativeEventListener) instance, NATIVE_EVENTS);
            instance = null;
        }
    }
    ...
    public static boolean openUriExternal(String targetURI,
        ...
        final Context context = activityContext != null ? activityContext : GoannaAppShell.getApplicationContext();
        ...
        if (!showPromptInPrivateBrowsing || activityContext == null) {
            ...
            // statically, but since this method is called from Goanna and I'm
    ...
    public static boolean hasHandlersForIntent(Intent intent) {
        try {
            return !GoannaAppShell.queryIntentActivities(intent).isEmpty();
        } catch (Exception ex) {
    ...
    public static String[] getHandlersForIntent(Intent intent) {
        final PackageManager pm = GoannaAppShell.getApplicationContext().getPackageManager();
        try {
            final List<ResolveInfo> list = GoannaAppShell.queryIntentActivities(intent);
    ...
    private static Intent getOpenURIIntentInner(final Context context, final String targetURI,
        ...
        if (!GoannaAppShell.isUriSafeForScheme(uri)) {
            return null;
        }
        ...
        if ("file".equals(scheme)) {
            ...
            final String mimeType2 = GoannaAppShell.getMimeTypeFromExtension(extension);
