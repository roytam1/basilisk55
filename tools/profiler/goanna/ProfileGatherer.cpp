...
ProfileGatherer::ProfileGatherer(GoannaSampler* aTicker)
  : mTicker(aTicker)
  , mSinceTime(0)
  , mPendingProfiles(0)
  , mGathering(false)
...
{
  // The GoannaSampler is going away. If we have a Promise in flight, we
  // should reject it.
  if (mPromise) {
  ...
  // Clear out the GoannaSampler reference, since it's being destroyed.
  mTicker = nullptr;
}
