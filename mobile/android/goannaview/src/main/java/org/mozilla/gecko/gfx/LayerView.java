...
public class LayerView extends FrameLayout {
    ...
    /* This is written by the Goanna thread and the UI thread, and read by the UI thread. */
    ...
    private class Compositor extends JNIObject {
        ...
        // Goanna thread sets its Java instances; does not block UI thread.
        @WrapForJNI(calledFrom = "any", dispatchTo = "goanna")
        /* package */ native void attachToJava(GoannaLayerClient layerClient,
        ...
        // Goanna thread creates compositor; blocks UI thread.
        @WrapForJNI(calledFrom = "ui", dispatchTo = "proxy")
        /* package */ native void createCompositor(int width, int height, Object surface);

        // Goanna thread pauses compositor; blocks UI thread.
        @WrapForJNI(calledFrom = "ui", dispatchTo = "current")
        /* package */ native void syncPauseCompositor();

        // UI thread resumes compositor and notifies Goanna thread; does not block UI thread.
        @WrapForJNI(calledFrom = "ui", dispatchTo = "current")
        ...
        private void destroy() {
            ...
            LayerView.this.mLayerClient.setGoannaReady(false);
    ...
    public void initializeView() {
        mLayerClient = new GoannaLayerClient(getContext(), this);
        if (mOverscroll != null) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (!mLayerClient.isGoannaReady()) {
            // If goanna isn't loaded yet, don't try sending events to the
    ...
    public boolean onHoverEvent(MotionEvent event) {
        ...
        // don't send it to goanna.
        if (event.getSource() == InputDevice.SOURCE_TOUCHSCREEN &&
            !GoannaAccessibility.isEnabled()) {
            return false;
        }
    ...
    private void attachCompositor() {
        ...
        if (GoannaThread.isStateAtLeast(GoannaThread.State.PROFILE_READY)) {
            ...
            GoannaThread.queueNativeCallUntil(GoannaThread.State.PROFILE_READY,
                    ...
                    GoannaLayerClient.class, mLayerClient,
    ...
    void updateCompositor() {
        ...
        if (mServerSurfaceValid && getLayerClient().isGoannaReady()) {
            mCompositorCreated = true;
    ...
    public void onFocusChanged (boolean gainFocus, int direction, Rect previouslyFocusedRect) {
        ...
        GoannaAccessibility.onLayerViewFocusChanged(gainFocus);
    }
